...
class PropertyOffer(models.Model):
    ...
    property_type_id = fields.Many2one(
        "property_type", related="property_id.property_type_id", string="Property Type"
    )
    ...
    _sql_constraints = [
        (
            "check_price",
            "CHECK(price >= 0)",
            "Property offer prices must be strictly positive",
        )
    ]
        ...
        for record in self:
            ...
            if record.validity:
                record.date_deadline = fields.Datetime.add(
                    offer_created, days=validity_day
                )
    ...
    def create(self, vals):
        ...
        if property.best_price >= vals.get("price"):
            raise UserError(
                "The offer price is not higher than the best offer received."
            )
